...
export function SchemaBuilder({ schema, onUpdate, onClose }: Props) {
  ...
  return (
    <aside className="w-72 flex-shrink-0 border-l border-gray-200 bg-[#f7f7f5] flex flex-col h-full overflow-hidden">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto py-2 px-2 space-y-1">
        {schema.columns.map((col, idx) => (
          <div key={col.id} className="bg-white rounded-md border border-gray-200 overflow-hidden">
            {/* Column row */}
            <div className="flex items-center gap-2 px-3 py-2">
              ...
              <span className="text-gray-400 flex-shrink-0">{TYPE_ICONS[col.type]}</span>
              <input
                value={col.name}
            ...
            {expanded === col.id && (
              <div className="border-t border-gray-100 px-3 py-3 space-y-3">
                      ...
                      {(col.options ?? []).map((opt) => (
                        <div key={opt} className="flex items-center gap-2 group">
                          ...
                          <button
                            ...
                            className="p-0.5 rounded text-transparent group-hover:text-gray-400 hover:!text-red-500 transition-colors"
                          >
                            <X size={12} />
                          </button>
                        </div>
                ...
                {col.type === 'relation' && (
                  <div>
                    <p className="text-xs font-medium text-gray-500 mb-1.5">Target database</p>
                    <select
                      value={col.targetPageId ?? ''}
                      onChange={(e) => updateColumn(col.id, { targetPageId: e.target.value || undefined })}
                      className="w-full text-xs border border-gray-300 rounded px-2 py-1.5 bg-white outline-none focus:ring-2 focus:ring-blue-300"
                    >
                ...
                {col.type === 'rollup' && (
                  <div className="space-y-2">
                    <div>
                      <p className="text-xs font-medium text-gray-500 mb-1">Relation column</p>
                      <select
                    ...
                    <div>
                      <p className="text-xs font-medium text-gray-500 mb-1">Target column (in linked database)</p>
                      <input
                        ...
                        className="w-full text-xs border border-gray-300 rounded px-2 py-1.5 outline-none focus:ring-2 focus:ring-blue-300"
                      />
                    </div>
                    <div>
      ...
      <div className="border-t border-gray-200 p-2">
        <button
          onClick={addColumn}
          className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-600 rounded hover:bg-[#ebebea] transition-colors"
        >
          <Plus size={14} />
          Add property
        </button>
