...
function Card({ row, nameColId, groupColId, allCols, onDelete, onEdit }: CardProps) {
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-3 group hover:shadow-sm transition-shadow">
      <div className="flex items-start gap-2">
        {editing ? (
          <input
            autoFocus
            defaultValue={name}
            className="flex-1 text-sm text-gray-800 outline-none border-b border-blue-300"
            onBlur={(e) => { onEdit(row.id, e.target.value); setEditing(false) }}
            onKeyDown={(e) => { if (e.key === 'Enter') (e.target as HTMLInputElement).blur() }}
          />
        ) : (
          <span
            className="flex-1 text-sm text-gray-800 cursor-text min-h-[20px]"
      ...
      {allCols.filter((c) => c.id !== nameColId && c.id !== groupColId).map((col) => {
        ...
        return (
          <div key={col.id} className="mt-1.5 text-xs text-gray-400">
            <span className="font-medium text-gray-500">{col.name}:</span>{' '}
...
export function BoardView({ schema, onUpdateRow, onDeleteRow, onAddRow }: Props) {
  ...
  if (!nameCol) {
    return <div className="py-10 text-center text-gray-400 text-sm">Add a column to get started.</div>
  }

  if (!groupCol) {
    return (
      <div className="py-10 text-center text-gray-400 text-sm">
        Board view requires a <span className="font-medium text-gray-600">Select</span> column to group by.<br />
        Open <span className="font-medium text-gray-600">Properties</span> and add one.
      </div>
    )
  }
  ...
  const groups = [
    { label: 'No status', value: null as string | null, color: 'bg-gray-100 text-gray-500' },
        ...
        return (
          <div key={group.label} className="flex-shrink-0 w-64">
            ...
            <button
              ...
              className="mt-2 w-full flex items-center gap-1.5 px-3 py-2 text-sm text-gray-400 hover:text-gray-600 hover:bg-white rounded-lg border border-dashed border-gray-200 hover:border-gray-300 transition-colors"
            >
              <Plus size={14} />
              Add card
            </button>
